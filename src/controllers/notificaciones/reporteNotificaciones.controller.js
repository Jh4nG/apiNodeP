...
const limit = "LIMIT ?, ?";
const sql = `WITH
             acm as (SELECT radicacion,username,despacho,etiqueta_suscriptor,expediente_digital FROM adm_clientes_misprocesos WHERE (#paramsWith) GROUP BY radicacion,despacho)

             SELECT SQL_CALC_FOUND_ROWS ap.idplanilla, ap.despacho, ap.radicacion, ap.notificacion, ap.proceso, ap.demandante, ap.demandado, ap.descripcion, ap.fechapublicacion, ap.departamento, ap.municipio, ap.corporacion, ap.despacho_a, REPLACE(ap.imagen,'Nota: ','') as nota, ap.ubicacion,ap.fechapublicacion as fechapublicacion
    ...
    if(valida.status){ // Se inserta
        ...
        if(result.length > 0 ){
            ...
            result.forEach((i,e)=>{
                ...
                whereWith.push(`(despacho = '${despacho}' AND radicacion = '${radicacion}')`);
            });
            ...
            const query = await connection.query(`${sql.replace('#paramsWith',`${whereWith.join(' OR ')}`)}
                                                  WHERE (${where.join(' OR ')}) 
