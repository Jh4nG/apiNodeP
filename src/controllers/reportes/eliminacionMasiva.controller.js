...
const deleteData = async (req,res)=>{
    try{
        ...
        if(result.affectedRows > 0){
            connection.end();
            return res.status(200).json({status : 200, msg : `EliminacionMasiva ${msgDeleteOk}`});
        }
        connection.end();
        return res.status(400).json({status : 400, msg : `${msgDeleteErr} EliminacionMasiva. ${msgTry}`});
    }catch(error){
